  ...
  for (size_t row=0; row < m; row++){
      for (size_t col=0; col<p; col++){
          ...
          for (size_t k=0; k<n; k++){
              sum += a.ptr[row*n+k]*b.ptr[k*p+col];
          }
          out->ptr[row*p+col] = sum;
      }
  }
  /// END YOUR SOLUTION
}
  ...
  for (size_t k=0; k<TILE; k++){
    for (int row=0; row<TILE; row++){
      for (int col=0; col<TILE; col++){
          out[row*TILE+col] += a[row*TILE+k]*b[k*TILE+col];
      }
    }
  }
  /// END YOUR SOLUTION
  ...
  for (int i=0; i<m/TILE; i++){
    for (int j=0; j<p/TILE; j++){
        for (int k=0; k<n/TILE; k++){
            AlignedDot(a.ptr+TILE*(i*n+k*TILE), b.ptr+TILE*(k*p+j*TILE), out->ptr+TILE*(i*p+j*TILE));
        }
    }
  }
  /// END YOUR SOLUTION
...
void ReduceMax(const AlignedArray& a, AlignedArray* out, size_t reduce_size) {
  ...
  /// BEGIN YOUR SOLUTION
    for (size_t out_idx=0; out_idx<out->size; out_idx++){
        size_t start_idx = out_idx * reduce_size;
        scalar_t max = 0;
        for (size_t offset=0; offset<reduce_size; offset++){
            scalar_t curr = a.ptr[start_idx+offset];
