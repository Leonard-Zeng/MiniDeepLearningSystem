...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
    ...
    for (size_t axis=0; axis<num_dim; axis++){
        length *= shape.data[axis];
    }

    if (gid < length){
        size_t mapped_idx = offset;
        size_t idx_trace = gid;
        for (int axis=num_dim-1; axis>=0; axis--){
            mapped_idx += (idx_trace%shape.data[axis])*strides.data[axis];
            idx_trace /= shape.data[axis];
        }
...
void EwiseSetitem(const CudaArray& a, CudaArray* out, std::vector<int32_t> shape,
    ...
    /// BEGIN YOUR SOLUTION
    CudaDims dim = CudaOneDim(out->size);
    EwiseSetitemKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, VecToCuda(shape), VecToCuda(strides), offset);
    /// END YOUR SOLUTION
}



__global__ void ScalarSetitemKernel(scalar_t* out, const scalar_t val, CudaVec shape,
                                    CudaVec strides, size_t offset){
    size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
...
void EwiseMaximum(const CudaArray& a, const CudaArray& b, CudaArray* out) {
    /**
     * Maximum together two CUDA array
     */
    CudaDims dim = CudaOneDim(out->size);
...
void ScalarMaximum(const CudaArray& a, scalar_t val, CudaArray* out) {
    /**
     * Maximum together a CUDA array and a scalar value.
     */
    CudaDims dim = CudaOneDim(out->size);
...
void ScalarEq(const CudaArray& a, scalar_t val, CudaArray* out) {
    /**
     * Eq together a CUDA array and a scalar value.
     */
    CudaDims dim = CudaOneDim(out->size);
...
void ScalarGe(const CudaArray& a, scalar_t val, CudaArray* out) {
    /**
     * Ge together a CUDA array and a scalar value.
     */
    CudaDims dim = CudaOneDim(out->size);
...
void ScalarPower(const CudaArray& a, scalar_t val, CudaArray* out) {
    /**
     * Power toPowerther a CUDA array and a scalar value.
     */
    CudaDims dim = CudaOneDim(out->size);
...
void EwiseTanh(const CudaArray& a, CudaArray* out) {
    /**
     * Tanh toTanhther two CUDA array
     */
    CudaDims dim = CudaOneDim(out->size);
...
__global__ void MatmulKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, uint32_t M, uint32_t N,
    ...
    __shared__ scalar_t ATile[TILE][TILE];
    __shared__ scalar_t BTile[TILE][TILE];

    size_t base_row = blockIdx.x * blockDim.x;
    size_t base_col = blockIdx.y * blockDim.y;

    size_t row_offset = threadIdx.x;
    size_t col_offset = threadIdx.y;

    size_t curr_row = base_row + row_offset;
    size_t curr_col = base_col + col_offset;

    scalar_t sum = 0;

    for (size_t tile_idx=0; tile_idx<(N-1)/TILE+1; tile_idx++){
        if (curr_row < M && tile_idx*TILE+col_offset < N){
            ATile[row_offset][col_offset] = a[curr_row*N+tile_idx*TILE+col_offset];
        }else{
            ATile[row_offset][col_offset] = 0;
        }

        if (tile_idx*TILE+row_offset < N && curr_col < P){
            BTile[row_offset][col_offset] = b[(tile_idx*TILE+row_offset)*P+curr_col];
        }else{
            BTile[row_offset][col_offset] = 0;
        }

        __syncthreads();

        for (size_t k=0; k<TILE; k++){
            sum += ATile[row_offset][k] * BTile[k][col_offset];
        }

        __syncthreads();
    }
    if (curr_row < M && curr_col < P)
        out[curr_row*P+curr_col] = sum;
}
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t n, size_t reduce_size) {
    size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
    if (gid < n){
        size_t start_idx = gid * reduce_size;
        scalar_t max = 0;
        for (size_t offset=0; offset<reduce_size; offset++){
            scalar_t curr = a[start_idx+offset];
...
__global__ void ReduceSumKernel(const scalar_t* a, scalar_t* out, size_t n, size_t reduce_size) {
    size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
    if (gid < n){
        size_t start_idx = gid * reduce_size;
        scalar_t sum = 0;
